...
SUPPORTED_TPROC_H_SCALE_FACTORS = (1, 10, 30, 100, 300, 1000, 3000, 10000, 30000, 100000)
...
def _load_rows() -> list[dict]:
    ...
    for fp in sorted(RESULTS_ROOT.rglob("*.json")):
        ...
        rows.append({
            ...
            "duration_minutes": cfg.get("duration_minutes"), "scale_factor": cfg.get("scale_factor"), "cpu_model": sys.get("cpumodel"), "cpu_count": sys.get("cpucount"),
...
def _section_html(title: str, rows: list[dict], empty_message: str, grouped_by_scale_factor: bool = False) -> str:
    visible = rows[:100]
    summary = _benchmark_summary_html(title, rows)
    if visible and grouped_by_scale_factor:
        body = _scale_factor_groups_html(visible)
    elif visible:
...
def _scale_factor_groups_html(rows: list[dict]) -> str:
    groups = {scale_factor: [] for scale_factor in SUPPORTED_TPROC_H_SCALE_FACTORS}
    for row in rows:
        scale_factor = row.get("scale_factor")
        if scale_factor in groups:
            groups[scale_factor].append(row)

    html_parts = []
    for scale_factor in SUPPORTED_TPROC_H_SCALE_FACTORS:
        group_rows = groups[scale_factor]
        if not group_rows:
            continue
        html_parts.append(f"<h3 class='sf-head'>SF{scale_factor}</h3>")
        for rank, row in enumerate(group_rows, 1):
            row["rank"] = rank
            html_parts.append(_row_html(row))
...
def _write_html(rows: list[dict]) -> None:
    ...
    top_h_html = _section_html("TPROC-H", tproc_h_rows, "No TPROC-H results yet.", grouped_by_scale_factor=True)
...
.hero-card{{background:#fff;border:1px solid var(--line);border-radius:18px;padding:18px 20px;box-shadow:0 1px 2px rgba(60,50,40,.06)}}
    .brandbar{{display:flex;align-items:flex-start;justify-content:space-between;gap:24px;margin-bottom:12px}}.brand-left{{display:flex;align-items:flex-end;gap:18px}}.brand-logo-link{{display:block;line-height:0}}.brand-hammerdb{{height:62px;width:auto;display:block}}.brand-tpc{{height:62px;width:auto;display:block;margin-top:0}}
...
<header class='hero'><div class='wrap'><div class='hero-card'><div class='brandbar'><div class='brand-left'><a class='brand-logo-link' href='https://www.hammerdb.com/'><img class='brand-hammerdb' src='assets/images/hammerDB-H-logo-FINAL.png' alt='HammerDB'></a><h1>HammerDB Result Artifacts</h1></div><a class='brand-logo-link' href='https://www.tpc.org/'><img class='brand-tpc' src='assets/images/tpclogo.png' alt='TPC'></a></div><div class='warn'>User-submitted, unaudited HammerDB result artifacts reviewed through GitHub.</div></div></div></header>
<main class='wrap'><section class='top-grid'><article class='card'><h2 style='margin:0 0 8px'>Star HammerDB on GitHub</h2><p>Help others discover HammerDB by starring the project.</p><a class='btn' href='https://github.com/TPC-Council/HammerDB'><span class='star'>★</span> Star HammerDB</a></article><article class='card'><h3 style='margin:0 0 8px'>Submission guidance</h3><p style='margin:0;color:var(--muted)'>To submit a result, open the benchmark report in HammerDB, select <strong>Save HammerDB result artifact</strong>, then select <strong>Share result artifact with TPC-OSS</strong>. The submission page validates the saved result artifact before continuing to GitHub.</p></article></section>
